...
class Node:
    ...
    A node is essentially an integer index, together with a boolean flag to distinguish "data" node
...
class CayleyComplex:
    ...
    quantum CSS code known as a quantum Tanner code.
    ...
    graph (V,E) must be bipartite, ``V = V_0 ∪ V_1``, such that (for example) nodes
    {g, agb} are in one partition, while nodes {ag, gb} are in the other partition.  The nodes
    V_0 and V_1 are then used as the source nodes of subgraph_x and subgraph_z.  The graph
    (V,E) is bipartite if:
    ...
    where ``Z_2 ~ {0,1}`` is the 2-element group (under addition), such that
    ``(a,1) * (g,i) = (ag,i+1)`` and ``(g,i) * (b,1) = (gb,i+1)``.  The complex generated by A
    and B after this modification is the "bipartite" Cayley complex constructed in
    ...
    where similarly to before ``(a,1,0) * (g,i,j) = (ag,i+1,j)`` and
    ``(g,i,j) * (b,0,1) = (gb,i,j+1)``.
...
class ChainComplex:
    ...
    "differential".  Neighboring boundary operators annihilate, in the sense that
    ``d_{j-1} d_j = 0``.
    ...
    def tensor_product(
        ...
        P_k of degree k can be written as a direct sum of tensor products ``A_i ⨂ B_j`` for
        which ``i+j=k``, that is:

        [1] ``P_k = ⨁_{i+j=k} A_i ⨂ B_j``.

        Elements of P_2, for example, can be written as vectors
