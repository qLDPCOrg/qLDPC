...
- `qldpc.circuits`: module for [`stim`](https://github.com/quantumlib/Stim) circuits and circuit utilities, including:
  ...
  - `get_transversal_ops`: logical tableaus and physical circuits for the SWAP-transversal logical Clifford gates of a code, constructed via the code automorphism method of [arXiv:2409.18175](https://arxiv.org/abs/2409.18175).  Uses [GAP](https://www.gap-system.org) by default, but can use [`MAGMA`](https://magma.maths.usyd.edu.au/magma) if passed `with_magma=True`.  (Warning: exponential complexity.)
  - `get_transversal_circuits`: find a SWAP-transversal physical circuit (if any) that implements a given logical Clifford operation in a code.  Uses [GAP](https://www.gap-system.org) by default, but can use [`MAGMA`](https://magma.maths.usyd.edu.au/magma) if passed `with_magma=True`.  (Warning: exponential complexity.)
